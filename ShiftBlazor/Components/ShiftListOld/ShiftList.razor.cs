...
namespace ShiftSoftware.ShiftBlazor.Components
{
        ...
        where T : ShiftEntityDTOBase, new()
    {
        ...
        {
            SettingManager.SetHiddenColumns(GetListIdentifier(), new List<ColumnState>());
        ...
        {
            //Grid?.PreventRender();
            var columns = ctx.Columns.Select(x => new ColumnState
            {
                Title = x.HeaderText,
                Visible = x.Visible,
            });

            var visibles = columns.Where(x => x.Visible).Select(x => x.Title).ToArray();
            var hiddens = columns.Where(x => !x.Visible).Select(x => x.Title).ToArray();
            ...
            //Grid?.PreventRender();
            SettingManager.SetHiddenColumns(GetListIdentifier(), columns.ToList());
        ...
        internal async Task OnLoadHandler()
        {
            ...
            {
                var hiddenColumns = SettingManager
                    .GetHiddenColumns(GetListIdentifier())
                    .Where(x => !x.Visible)
                    .Select(x => x.Title)
                    .ToArray();
