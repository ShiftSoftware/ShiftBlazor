...
window.CloseFileExplorerDialogs = function (id) {
  document
    .getElementById(id)
    ?.querySelectorAll(".e-dialog .e-dlg-closeicon-btn")
    .forEach((x) => x.click && x.click())
}
...
window.handleKeydown = function (e) {
  ...
  if (
    (e.altKey && e.code != null && !e.code.includes("Alt")) ||
    e.code == "Escape"
  ) {
...
window.SetAsSortable = function (id) {
  ...
  if (element && grid.children[0].getAttribute("draggable") == null) {
    new Sortable(grid, {
      ...
      onMove: function (e) {
        if (
          element.classList.contains("readonly") ||
          e.related.classList.contains(addButtonClass.slice(1))
        ) {
          return false
        }
      },
      onEnd: function (e) {
        let order = [
          ...grid.querySelectorAll(`.mud-grid-item:not(${addButtonClass})`),
        ].map((x) => x.dataset.guid)
        DotNet.invokeMethod("ShiftSoftware.ShiftBlazor", "ReorderGrid", {
          Key: id,
          Value: order,
        })
      },
...
window.fixStickyColumn = function (gridId) {
  ...
  var headRow = grid.querySelector(
  ...
  for (var cell of leftCells) {
    css.push(
      `#${gridId} .mud-table-container .mud-table-row .mud-table-cell.sticky-left:nth-child(${
        cell.cellIndex + 1
      }) { left: ${offset}px}`
    )
  ...
  for (var cell of rightCells.reverse()) {
    css.push(
      `#${gridId} .mud-table-container .mud-table-row .mud-table-cell.sticky-right:nth-child(${
        cell.cellIndex + 1
      }) { right: ${offset}px}`
    )
    var cellWidth = cell.getBoundingClientRect().width
...
window.fixAllStickyColumns = function () {
  document
    .querySelectorAll("[id^='Grid-']")
    .forEach((x) => fixStickyColumn(x.id))
}
